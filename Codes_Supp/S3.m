%figures for S1
%plots mean end time, probability of severing, and severing time vs
%concentration of tubulin, compares theory and simulation
...
%parameters
kt=.11; % rate of tubulin incorporation
ks=.54;  % rate of tubulin removal
L=25; %(N_d), max tubulin removal before severing
cts=0:.2:5;
...
% 
figure;
errorbar(cts,endtime2,endtimestd,'-o','DisplayName','simulation');
%xlabel('concentration of tubulin');
%ylabel('mean end time');
%title("kt="+num2str(kt)+"  "+"ks="+num2str(ks)+" "+"x0="+num2str(x0)+" "+"L="+num2str(L));
hold on;
plot(cts,B,'DisplayName','theory');
legend;
% 
figure;
 errorbar(cts,sevtime2,sevtimestd,'-o','DisplayName','simulation');
 hold on;
